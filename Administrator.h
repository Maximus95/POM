...
private:
	...
	void addDailyItemWise(){
		...
		bool exist = false;
		int itemID, totalSales, salesManagerID, quantity;
		Item item;
				...
				if ((item.getQuantity() - totalSales) >= 0) {
					...
					for (unsigned int i = 0; i < users->size(); i++){
						if (users->at(i).getUserID() == salesManagerID) {
							...
							if (users->at(i).getCategory() == "SalesManager") {
								salesManagerID = users->at(i).getUserID();
								break;
							}
							else {
								exist = false;
					...
					if (!exist) {
						...
						if (users->size() != 0) {
							...
							itemWise.setSalesManagerID(salesManagerID);
	...
	void addPurchaseOrder(){
		...
		int purchaseRequisitionID, purchaseManagerID;
		PurchaseRequisition purchaseRequisition;
		bool exist = false;
		PurchaseRequisitionDAO purchaseRequisitionDAO;
		UsersDAO usersDAO;
						...
						for (unsigned int i = 0; i < users->size(); i++){
							if (users->at(i).getUserID() == purchaseManagerID) {
								...
								if (users->at(i).getCategory() == "PurchaseManager") {
									...
									purchaseOrder.setPurchaseRequisitionID(purchaseRequisitionID);
									exist = true;
